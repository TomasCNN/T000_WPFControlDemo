# 004003004_WPF ButtonBase 基类官方类定义逐行深度解析（.NET 8 最新版）
...
静态构造函数是 ButtonBase 最关键的部分，负责所有路由事件和依赖属性的注册。

### 1. `ClickEvent` 路由事件注册（核心）
...
- **`EventManager.RegisterRoutedEvent`**：WPF 路由事件的标准注册方法
- **`nameof(Click)`**：事件名称，必须与公共事件属性名称一致
- **`RoutingStrategy.Bubble`**：路由策略为**冒泡路由**，事件从触发元素向上冒泡到根元素
- **`typeof(RoutedEventHandler)`**：事件处理程序的类型
- **`typeof(ButtonBase)`**：事件的所有者类型

#### 核心设计意义：

- 路由事件允许事件在可视化树中传播，而不仅仅是在触发元素上处理
- 冒泡路由策略符合按钮的交互习惯，父容器可以统一处理子按钮的点击事件
- 这是 WPF 事件系统与传统 WinForms 事件系统最本质的区别之一

### 2. `ClickModeProperty` 注册
...
- **`public static readonly RoutedEvent ClickEvent`**：静态只读路由事件字段，是 WPF 路由事件的标准定义方式
- **`event RoutedEventHandler Click`**：公共事件包装器，提供标准的事件订阅 / 取消订阅语法
- **`add => AddHandler(ClickEvent, value)`**：添加事件处理程序到路由事件
- **`remove => RemoveHandler(ClickEvent, value)`**：从路由事件中移除事件处理程序

#### 核心特性：

- **路由策略**：冒泡路由（Bubble），事件从触发元素向上传播到根元素
- **触发时机**：根据`ClickMode`属性的设置，在按下、释放或悬停时触发
- **与 Command 的关系**：Click 事件触发后，会自动执行绑定的 Command 命令
- **可取消性**：不能直接取消 Click 事件，但可以通过设置`e.Handled = true`阻止事件继续冒泡

#### 工业最佳实践：

- ✅ MVVM 模式下优先使用 Command 绑定
- ✅ 只有在简单的 UI 交互时才使用 Click 事件
- ❌ 不要在 Click 事件中编写复杂业务逻辑
