# 002002001_DependencyObject 类


> **摘要**：`DependencyObject` 是 WPF 依赖属性系统的根基，为 UI 对象提供了**注册、存储、值解析、变更通知、元数据管理**五大核心能力。本文从生活化类比入手，通俗拆解依赖属性相比普通 C# 属性的本质差异——默认值共享省内存、按优先级自动决定最终值、属性变化自动通知绑定与动画，并梳理了哪些对象需要继承它、哪些不需要。一句话：它是 WPF 对象“智能属性”的管家，与负责线程安全的 `DispatcherObject` 分工协作，共同撑起了 WPF 灵活而强大的 UI 体系。
...
​	这个 “管家” 的核心工作是管理依赖属性，主要做 5 件事，每一件都对应 WPF 的核心特性：

### 1. 注册依赖属性：给“管家”新增可管理的属性

​	就像给管家说“新增一个‘背景色’的管理项”，开发者通过 `DependencyProperty.Register()` 方法注册，告诉管家：这个属性名称叫什么、是什么类型、属于哪个对象、默认值是什么、值变更后要做什么。
...
```csharp
// 多个按钮实例，未设置背景色时共用同一份默认值
Button btn1 = new Button();                     // 使用默认背景色（如蓝色）
Button btn2 = new Button();                     // 同样使用默认背景色，不额外存储
Button btn3 = new Button();
btn3.SetValue(Button.BackgroundProperty, Brushes.Red); // 显式赋值，单独存储

// btn1 和 btn2 的 Background 值都指向内部共享的一份“默认蓝色”，
// 不会为每个对象复制一份字段值，节省大量内存；
// btn3 因显式设值，才会在字典中生成独立的条目存储“红色”。
```
### 3. 自动解析属性值：按优先级找最终值
...
```csharp
// 注册依赖属性时指定变更回调，管家自动触发通知
public static readonly DependencyProperty StatusTextProperty =
    DependencyProperty.Register(
        "StatusText", typeof(string), typeof(MyControl),
        new PropertyMetadata("空闲", OnStatusTextChanged));

private static void OnStatusTextChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
{
    // 属性值变化时，管家回调自动执行，无需手动编写通知逻辑
    Console.WriteLine($"状态从 {e.OldValue} 变为 {e.NewValue}");
}

// 使用：修改依赖属性值，自动触发变更通知
MyControl control = new MyControl();
control.StatusText = "采集中";    // 输出：状态从 空闲 变为 采集中
control.StatusText = "已完成";    // 输出：状态从 采集中 变为 已完成
```
### 5. 元数据管理：给属性加“附加规则”
