# 005006004_WPF `ListBox` 工业场景实战案例合集

以下案例全部贴合工业上位机、设备监控、生产管理等真实业务场景，从基础绑定、多选交互、样式定制到高性能虚拟化、拖拽排序、分组展示逐步深入，每个案例明确标注对应 `ListBox` 核心特性，可直接复用至工业项目中。

------

## 案例 1：设备监控主从列表（MVVM 单选联动）

### 场景说明

左侧设备列表展示车间所有设备的基础状态，选中某台设备后右侧自动加载实时参数与运行详情，是工业监控系统最经典的主从布局。
...
## 案例 2：报警批量确认（扩展多选 + 批量操作）

### 场景说明

报警列表支持 Ctrl 点选、Shift 连选，顶部工具栏批量执行确认、导出、删除操作，是工业报警系统的标配功能。

### 对应核心特性

- `SelectionMode="Extended"` 扩展多选模式
- `SelectedItems` 选中集合
- `SelectAll()` / `UnselectAll()` 内置批量方法
...
> 💡 MVVM 多选方案：纯 MVVM 架构下可通过自定义附加属性监听 `SelectionChanged`，将 `SelectedItems` 同步到 ViewModel 集合，避免后台代码直接操作控件。

------

## 案例 3：工业深色主题列表（样式深度定制）

### 场景说明

适配工控机深色操作界面，自定义选中态、悬浮态、交替行样式，区分激活 / 失焦选中效果，降低现场操作眩光。

### 对应核心特性

- `ItemContainerStyle` 容器样式全定制
- `AlternationCount` 交替行计数
- `MultiTrigger` 组合条件触发器
...
上万条历史报警记录，要求滚动流畅、内存占用低，支持搜索关键字后自动滚动定位到匹配项。
...
调整生产工序、配方步骤的执行顺序，通过鼠标拖拽上下移动位置，是工艺配置界面的常用交互。

### 对应核心特性

- `AllowDrop` 拖放开启
- 鼠标按下 / 移动 / 释放事件配合
- `ObservableCollection.Move` 数据驱动 UI 自动同步
- 内置滚动支持，拖拽到边缘自动滚动
...
2. **不要外层包裹 `ScrollViewer`**：`ListBox` 内置滚动，外层嵌套会导致滚动冲突、虚拟化失效、性能下降。
3. **单选用绑定，多选用附加属性**：单选直接绑定 `SelectedItem` 到 ViewModel；多选通过附加属性同步 `SelectedItems`，保持 MVVM 架构整洁。
4. **批量操作用内置方法**：全选 / 全不选使用 `SelectAll()` / `UnselectAll()`，性能远高于循环逐条设置。
5. **样式与数据分离**：选中、交替行、状态色通过样式触发器实现，不要在数据模型中添加 UI 相关属性。
6. **工业界面优先状态可视化**：通过颜色圆点、图标直观展示设备运行、报警等级等状态，提升现场操作效率。